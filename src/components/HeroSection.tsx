...
function HeroSection() {
 ...
 const handleMess = () => {
  window.open("https://m.me/jhonwell.espanola", "_blank")
 }
    ...
    <div className='mt-10 flex flex-col gap-10 lg:mt-0 2xl:mt-7'>
       <section className={`flex flex-col relative gap-5 md:border ${theme === "Dark" ? "md:border-gray" : "md:border-[#222]"} md:rounded-xl md:p-10 lg:gap-8 lg:rounded-2xl xl:gap-5 xl:p-12 2xl:p-16`}>
        ...
        <div className='flex gap-4 text-sm xs:mt-3 xs:gap-6 2xl:text-lg 2xl:gap-8'>
          <button onClick={handleMess} className={'hero-btns flex p-3 px-4 cursor-pointer rounded-md items-center gap-2 bg-violet-dark text-white xs:px-6 xl:gap-3 2xl:px-8'}>
            <img className='w-4 xl:w-5 2xl:w-6' src={messenger} alt="" />
            <span>Message</span>
          </button>

          <button onClick={handleRsume} className={`hero-btns flex overflow-hidden pr-5 p-3 px-4 ${theme === "Dark" ? "bg-[#444]" : "bg-white"}  rounded-md cursor-pointer items-center shadow-light gap-2 relative before:absolute before:right-0 before:w-[7px] before:h-full before:bg-violet-dark xs:px-6 xl:gap-3 2xl:px-8 2xl:py-4`}>
  ...
  return (
    <div className='relative services overflow-hidden rounded-lg lg:rounded-xl'>
      <img src={imgSrc} alt="" />
       ...
       <div className="absolute justify-between text-white text-sm p-3 flex flex-col inset-0 bg-[rgba(0,0,0,0.4)] xs:p-5 md:p-4 lg:p-5 lg:px-7 2xl:p-10">
