...
const RecentProject = () => {
    ...
    return <div>
        ...
        <div className='grid gap-5 mt-10 justify-center xs:grid-cols-2 xs:gap-2 lg:gap-5 lg:mt-14 xl:mt-20 xl:gap-10'>
          {
...
const ProjectBox:React.FC<ProjectType> = ({imgProj, category, title, url}) => {
    ...
    return (
        <div className={`relative flex justify-center items-center before:absolute before:bg-darker-overlay before:inset-0 overflow-hidden rounded-lg max-w-[20em] xs:max-w-full group lg:before:bg-transparent hover:lg:before:bg-dark-overlay hover:lg:before:bg-linear-[0deg,#7B11CD_.2%,transparent_85%] lg:before:duration-500 lg:before:transition-all`}>
            ...
            <img className='w-full' src={imgProj} alt="" />
            
            <h3 className='poppins-semibold absolute w-fit text-white text-[1.2em] lg:text-[1.6em] lg:translate-y-[50px] transition-all duration-500 delay-[20ms] lg:opacity-0 lg:group-hover:translate-y-0 lg:group-hover:opacity-100 xl:text-[1.9em] 2xl:text-[2.3em]'>{title}</h3>
        </div> 
    )
}
